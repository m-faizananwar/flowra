const { compactJson } = require("./json-utils");
...
function buildJiraPrompt(context) {
  ...
  if (context.jiraCards && Object.keys(context.jiraCards).length > 0) {
    ...
    for (const [key, card] of Object.entries(context.jiraCards)) {
      jiraCardContext += `\n• ${key}: "${card.summary}" | Status: ${card.status} | Priority: ${card.priority || "Medium"}`;
      if (card.assigneeName) jiraCardContext += ` | Assignee: ${card.assigneeName}`;
      if (card.description) jiraCardContext += `\n  Description: ${card.description.slice(0, 200)}`;
    }
  } else {
