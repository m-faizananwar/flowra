...
function weightedTotal(metricScores, metrics) {
  const weights = new Map(metrics.map((metric) => [metric.id, Number(metric.weight || 1)]));
  let weighted = 0;
  let totalWeight = 0;

  for (const metricScore of metricScores) {
    const weight = weights.get(metricScore.metric_id) || 1;
...
module.exports = new ApprovalBuffer();
