...
class BotManager {
    ...
    try {
      const { data: intMember, error: profileError } = await supabase
        ...
        .upsert({
          ...
          avatar_url: (rawData as any).avatar_url || (rawData as any).photo_url || null,
      ...
      if (intMember) {
        ...
        if (!memberId && userId) {
          ...
          if (memberId) {
            ...
            const incomingAvatar = (rawData as any).avatar_url || (rawData as any).photo_url;
            if (incomingAvatar) (updateData as any).avatar_url = incomingAvatar;
  ...
  async saveMessage(integrationId, externalId, sender, content, channelExternalId = "unknown", channelName = "unknown", rawData = {}, externalUserId = null, username = null, displayName = null) {
    try {
      ...
      for (const service in this.integrations) {
        const found = Object.values(this.integrations[service]).find((i: any) => i.id === integrationId);
        if (found) {
          integration = found;
          break;
        }
      ...
      if (externalUserId) {
        ...
        memberId = await this.anchorMember(integrationId, userId, externalUserId, username || sender, displayName || sender, { 
          ...(raw.user || {}), ...(raw.from || {}), avatar_url: raw.avatar_url || null 
        });
      }
  ...
  startDiscord(token, integrationId) {
    if (!token) return;
    const client = new Client({ intents: [GatewayIntentBits.Guilds, GatewayIntentBits.GuildMessages, GatewayIntentBits.MessageContent, GatewayIntentBits.DirectMessages, GatewayIntentBits.GuildMembers], partials: ["CHANNEL" as any] });
    ...
    client.on("messageCreate", async (message) => {
      ...
      await this.saveMessage(integrationId, message.id, message.member?.displayName || message.author.username, message.cleanContent, channel.id, channel.name || "dm", { 
        ...(message.toJSON ? message.toJSON() : message) as any, 
  ...
  async handleGitHubEvent(event, payload, deliveryId = null) {
    try {
      const { default: githubEvents } = await import('./intelligence/github-events');
      ...
      if (event === "installation" || event === "installation_repositories") {
        ...
        if (integrations?.length > 0) {
          const { default: githubSync } = await import('./github-sync');
